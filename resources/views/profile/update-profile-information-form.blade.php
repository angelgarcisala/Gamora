<x-form-section submit="updateProfileInformation">
    ...
    <x-slot name="form">
        ...
        @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
            <div x-data="{photoName: null, photoPreview: null}" class="col-span-6 sm:col-span-4">
                ...
                <x-label for="photo" value="{{ __('Foto') }}" class="text-[#FFFFFF]" />
                ...
                <x-input-error for="photo" class="mt-2 text-[#FFFACD]" />
            </div>
        @endif

        <!-- Name -->
        <div class="col-span-6 sm:col-span-4">
            <x-label for="name" value="{{ __('Nombre') }}" class="text-[#FFFFFF]" />
            <x-input
                id="name"
                type="text"
                ...
                autocomplete="name"
            />
            <x-input-error for="name" class="mt-2 text-[#FFFACD]" />
        </div>

        <!-- Email -->
        <div class="col-span-6 sm:col-span-4">
            <x-label for="email" value="{{ __('Correo electrónico') }}" class="text-[#FFFFFF]" />
            <x-input
                id="email"
                type="email"
                class="mt-1 block w-full bg-white/10 text-white placeholder-white/60 border border-white/20 rounded-md focus:bg-white/20 focus:border-white focus:ring-white"
            ...
            />
            <x-input-error for="email" class="mt-2 text-[#FFFACD]" />

            @if (Laravel\Fortify\Features::enabled(Laravel\Fortify\Features::emailVerification()) && ! $this->user->hasVerifiedEmail())
                <p class="text-sm mt-2 text-[#FFFFFF]">
    ...
    <x-slot name="actions">
        ...
        <x-button
            ...
            wire:target="photo"
            class="bg-purple-800 hover:bg-purple-900 text-[#FFFFFF]"
        >
            {{ __('Guardar') }}
        </x-button>
    </x-slot>
